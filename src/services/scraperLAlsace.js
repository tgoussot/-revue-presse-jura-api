...
exports.searchArticles = async ({ keyword, startDate, endDate }) => {
  ...
  let found_older_article = false; // Comme dans les scripts originaux
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    try {
      ...
      for (let i = 0; i < articlesElements.length; i++) {
        ...
        totalArticlesFound++;
        console.log(`Article ${i}: Lié à l'Alsace, récupération du contenu complet`);
        ...
        try {
          ...
          if (dateInRange(metadata.date, startDate, endDate)) {
            ...
            console.log(`Article ${i}: Départements: ${metadata.departements.join(', ')}`);
