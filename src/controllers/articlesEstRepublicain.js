const scraperService = require('../services/scraperEstRepublicain');
...
exports.searchArticles = async (req, res, next) => {
  try {
    ...
    console.log('====== RECHERCHE D\'ARTICLES EST RÉPUBLICAIN ======');
    ...
    const articles = await scraperService.searchArticles({
      keyword,
      startDate,
      endDate
    });
    ...
    if (articles.length > 0) {
      ...
      articles.slice(0, 5).forEach((article, index) => {
        console.log(`- Article ${index+1}: date=${article.date}, titre=${article.title}, départements=${article.departements.join(',')}`);
      });
    }
    
    res.json(articles);
  } catch (error) {
    console.error('Erreur lors de la recherche d\'articles dans l\'Est Républicain:', error);
    next(error);
  }
}; 